...
export default function SecondGame() {
 ...
 const handleVerify = (value: number) => {
  ...
  if (isCorrect) {
    ...
    setIsWrong(false);
    if (rounds < 9) handleGetValues(); // evita gerar nova rodada após a 10ª
  } else {
    setIsWrong(true);
              ...
              <div className="flex flex-col items-center justify-center">
                <div className="flex gap-1">
                  {Array.from({ length: 3 }).map((_, index) => (
                    <Heart
                      key={index}
                      className={`w-6 h-6 ${index < lives ? 'text-red-500 opacity-100' : 'text-red-300 opacity-50'}`}
                    />
                  ))}
                </div>
                <Image src="/character.svg" alt="Personagem" width={100} height={100} />
              </div>
            </div>

            <div className="flex flex-row w-1/3 items-center justify-around gap-2">
              <h1 className={`${luckiestGuy.className} text-center text-5xl tracking-wider`}>
                {n1} <span className='text-[#227C9D]'>x</span> {n2} = <span className="text-[#227C9D]">?</span>
              </h1>
            </div>
      ...
      <AnimatePresence>
        {isWrong && (
          <div className="fixed inset-0 backdrop-blur-sm bg-black/20 flex flex-row gap-3 items-center justify-center z-50">
            ...
            >
              <Image src={'/lose.svg'} alt="Mascote Tente novamente" width={300} height={300} className="object-contain mb-4" priority />
              <Image src={'/lose_dialog.svg'} alt="Dialogo Tente novamente" width={500} height={500} className="object-contain mb-4" priority />
            </motion.div>
          </div>
        )}
      </AnimatePresence>
