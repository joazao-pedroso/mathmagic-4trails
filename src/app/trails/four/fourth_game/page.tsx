...
export default function FourGame() {
  ...
  function handleGetValues() {
    const n1 = Math.floor(Math.random() * 40) + 1; // n2: de 2 a 10
    const n2 = Math.floor(Math.random() * 40) + 1; // resposta correta: de 1 a 10
    const correct = n1 + n2; // n1 = resposta * divisor
    ...
    setN1(n1); // dividend = n1
    setN2(n2); // divisor = n2
    setCorrectAnswer(correct); // correct = n1 / n2
  }
  ...
  const handleVerify = (value: number) => {
    ...
    if (correct) {
      setEnemyLives((prev) => {
        ...
        setTotalAcertos((prevAcertos) => [...prevAcertos, `${n1}+${n2}`]);
        return newEnemyLives;
      });
    } else {
      setLives((prev) => prev - 1);
      setTotalErros((prevErros) => [...prevErros, `${n1}+${n2}`]);
    }
  };
        ...
        <div className="container flex flex-col gap-3 bg-white rounded-lg shadow-md mx-auto mt-5 p-5 w-250 h-160">
          ...
          <div className="text-center h-2/3 flex flex-col justify-center items-center bg-white rounded-lg shadow-lg">
            <div className="flex gap-3 flex-col items-center justify-center h-full w-full mt-4">
              ...
              <p className="text-lg font-semibold">Qual é o resultado da adição?</p>
              </div>
          </div>
        </div>
      </div>
